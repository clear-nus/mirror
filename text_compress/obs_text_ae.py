...
class AutoEncodersUni(torch.nn.Module):
    ...
    def get_latent_dist(self, lidar_obs, speed, h_direction, v_direction):
        # h_value = torch.cat(6 * [torch.zeros_like(speed).to(speed.device)], dim=-1)
        # h_value[..., self.h_direction_list[h_direction]] = 1.0
        # v_value = torch.cat(2 * [torch.zeros_like(speed).to(speed.device)], dim=-1)
        # v_value[..., self.v_direction_list[v_direction]] = 1.0
