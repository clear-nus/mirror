...
def generate_text_embed(obs: torch.Tensor, tokenizer, gpt_model, gpt_device="cuda:0", device="cuda:1"):
    ...
    # print(text)
    return gpt_embed, text_token, text
...
class CarlaEnv():
    ...
    def random_shooting(self, current_robot_state, pre_agent_state, current_obs):
        batch_size = 20000
        action_mask = []
        action_mask += [torch.randint(low=0, high=2, size=(batch_size, 1)).repeat(1, 3)]
        action_mask += [torch.randint(low=0, high=2, size=(batch_size, 1)).repeat(1, 3)]
        action_mask += [torch.randint(low=0, high=2, size=(batch_size, 1)).repeat(1, 3)]
        action_mask += [torch.randint(low=0, high=2, size=(batch_size, 1)).repeat(1, 3)]
        action_mask += [torch.randint(low=0, high=2, size=(batch_size, 1)).repeat(1, 3)]
        ...
        action_mask = torch.cat(action_mask + [torch.ones(size=(batch_size, 5))], dim=-1)
        action_mask = torch.cat(
            [action_mask, torch.randint(low=0, high=2, size=(batch_size, 1)).repeat(1, 1)], dim=-1)
        # action_mask[:] = 1.0
        # print(current_state.shape)
        # print(action_mask)

        pre_agent_state = torch.tensor(self._latent_state_agent).type(torch.float)
        pre_agent_action, _ = self.model.policy(pre_agent_state)
        pre_agent_action = torch.tanh(pre_agent_action)
        pre_agent_action = self._action

        latent_state_env = torch.tensor(self._latent_state_env).type(torch.float)
        text_embed_torch = self.model.autoencoder.decoder_row_text(latent_state_env)

        limited_view_mask = torch.ones_like(action_mask[..., :]).to(action_mask.device)
        limited_view_mask[..., 0:4] = 0.0
        limited_view_mask[..., -4 - 6:-6] = 0.0

        latent_state_agent = self.model.autoencoder.get_latent_state_dist(
            description=current_obs.unsqueeze(0).repeat(batch_size, 1) * action_mask[..., :-1] * limited_view_mask[..., :-1],
            text_embeds=text_embed_torch.unsqueeze(0).repeat(batch_size, 1) * action_mask[..., -1:] * limited_view_mask[..., -1:],
            pre_state=pre_agent_state.unsqueeze(0).repeat(batch_size, 1),
            action=pre_agent_action.unsqueeze(0).repeat(batch_size, 1),
            mask=action_mask * limited_view_mask).mean

        ase_value = (latent_state_agent - current_robot_state.unsqueeze(0)).pow(2).sum(dim=-1) + action_mask.sum(-1) * 0.85
        # print(accumulated_q_value.size())
        min_id = torch.argmin(ase_value, dim=0)

        opti_action_mask = action_mask[min_id]
        # print(ase_value[min_id])
        # print(opti_action_mask.detach().cpu().numpy())
    ...
    def get_current_human_action(self):
        return torch.tensor(self._action)
    ...
    def get_current_text_embed(self):
        latent_state = torch.tensor(self._latent_state_env)
        return self.model.autoencoder.decoder_row_text(latent_state)
    ...
    def reset(self):
        ...
        text_embed_env, _, text = generate_text_embed(env_state_torch, self.tokenizer, self.gpt_model)
        ...
        mask = torch.ones(size=(24,)).type(torch.float)
        self._action_mask = mask.detach().numpy().copy()

        text_embed_agent, _, text = generate_text_embed(env_description_agent_torch, self.tokenizer, self.gpt_model)
        ...
        env_obs = obs.copy()
        return self._state.copy(), collided, env_obs
    ...
    def step_communication(self, action):
        ...
        for _ in range(self.frame_skip):
            self._state, reward, collided, done, text_token, text_embed, env_state= self.forward(self._state, action)
            rewards.append(reward)
            if collided != 0.0:
                collideds = 1.0
        reward = np.mean(rewards)

        return self._state.copy(), reward, collideds, done, text_token, text_embed, env_state

    def get_current_latent_state(self, init_flag):
        if init_flag:
            latent_state_env = torch.tensor(self._latent_state_env).type(torch.float)
            latent_state_agent = torch.tensor(self._latent_state_agent).type(torch.float)
            text_embed_torch = None
        else:
            action_mask_torch = torch.tensor(self._action_mask).type(torch.float)
            env_mask = torch.ones_like(action_mask_torch).to(action_mask_torch.device).type(torch.float)
            env_mask[..., -1:] = 0
            env_state_torch = torch.tensor(self._env_state).type(torch.float)
            latent_state_env_torch = torch.tensor(self._latent_state_env).type(torch.float)
            text_embed_torch = torch.zeros(size=(768*10,))
            latent_state_env = self.model.get_state_representation(
                observation=env_state_torch * env_mask[..., :-1],
            ...
            latent_state_agent = torch.tensor(self._latent_state_agent).type(torch.float)
            latent_state_agent = self.model.autoencoder.get_latent_state_dist(
                description=env_state_torch * action_mask_torch[:-1] * limited_view_mask[..., :-1],
                text_embeds=text_embed_torch * action_mask_torch[-1:] * limited_view_mask[..., -1:],
                pre_state=latent_state_agent[:self.latent_size],
                action=self._action,
                mask=action_mask_torch * limited_view_mask).mean

            # print((latent_state_env-latent_state_agent).pow(2).sum(-1))
        return latent_state_env, latent_state_agent
    ...
    def forward(self, current_state: np.array, action_mask: np.array):
        ...
        text_embed, text_token, text = generate_text_embed(self._env_state, self.tokenizer, self.gpt_model)
        ...
        return self._state, reward, collided, done, text_token, text_embed_torch.detach().cpu().numpy(), env_state
