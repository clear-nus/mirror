...
def game_loop(args):
    ...
    try:
        ...
        env = CarlaEnv(render_display=args.render, render_fog=False, expt_stage='warmup-clear',
        ...
        # Warm up with clear weather
        while True:
            obs, _, done = env.resetWarmup()
            if not done:
                break
