...
class BehaviorAgent(Agent):
    ...
    def _overtake(self, location, waypoint, vehicle_list, distance, construction_dist, other_cars_lanes):
        ...
        if len(other_cars_lanes) == 10:      # No construction
            cars_lane_index1 = other_cars_lanes[-1] - 1
            cars_lane_index2 = other_cars_lanes[-2] - 1
            # If ego car is at other cars lane and other car is within lidar range
            if (curr_lane_index == cars_lane_index1 or curr_lane_index == cars_lane_index2) and len(vehicle_list) > 0:
                if curr_lane_index == 1:
                    next_wpt = next_wpt.get_left_lane()
                elif curr_lane_index == 2:
                    next_wpt = next_wpt.get_right_lane()
                next_wpt = next_wpt.next(5.)[0]
                target_speed = 25.
            else:
                next_wpt = next_wpt.next(5.)[0]
        elif len(other_cars_lanes) == 11:      # Construction present
            construction_lane_index = other_cars_lanes[-1] - 1
            if construction_lane_index[construction_lane_index] < 1.0:
                goal_lane = 3 if construction_lane_index == 0 else 0
                while curr_lane_index != goal_lane:
                    if curr_lane_index < goal_lane:
                        next_wpt = next_wpt.get_right_lane()
                        curr_lane_index += 1
                    elif curr_lane_index > goal_lane:
                        next_wpt = next_wpt.get_left_lane()
                        curr_lane_index -= 1
                next_wpt = next_wpt.next(5.)[0]
                target_speed = 25.
