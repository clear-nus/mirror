...
class BehaviorAgentBackCar(Agent):
    """
    ...
    def __init__(self, vehicle, ignore_traffic_light=False, behavior='normal'):
        ...
        super(BehaviorAgentBackCar, self).__init__(vehicle)
    ...
    def _overtake(self, location, waypoint):
        ...
        nearby_vehicle_list = [v for v in vehicle_list if dist(v) < 20 and v.id != self.vehicle.id]

        if len(nearby_vehicle_list) > 0:
            if len(nearby_vehicle_list) == 2:
                next_wpt = next_wpt.next(10.)[0]
            else:
                for i in range(len(nearby_vehicle_list)):
                    if waypoint.lane_id == self._map.get_waypoint(nearby_vehicle_list[i].get_location()).lane_id:
                        if waypoint.lane_id == -2:
                            next_wpt = next_wpt.get_right_lane()
                            next_wpt = next_wpt.next(7.)[0]
            ...
            debug.draw_box(carla.BoundingBox(next_wpt.transform.location, carla.Vector3D(0.5, 0.5, 0.5)),
                           next_wpt.transform.rotation, 0.05, carla.Color(255, 0, 0, 0), 1.0)

        # if scenario_mode == 0:
        # if len(nearby_vehicle_list) > 0:
        #     for i in range(len(nearby_vehicle_list)):
        #         if self._map.get_waypoint(nearby_vehicle_list[i].get_location()).lane_id == waypoint.lane_id:
        #             if curr_lane_index == 1 and to_speed_up:
        #                 if lanes_obs[0] == 1.:
        #                     next_wpt = next_wpt.get_left_lane()
        #                     target_speed = 25.
        #                 else:
        #                     target_speed = 18.
        #             elif curr_lane_index == 1 and not to_speed_up:
        #                 next_wpt = next_wpt.get_left_lane()
        #                 target_speed = 20.
        #             elif curr_lane_index == 0:
        #                 if lanes_obs[1] < 1.:
        #                     target_speed = 40.0
        ...
        #                 if to_speed_up:
        #                     next_wpt = self._map.get_waypoint_xodr(road_id=45, lane_id=next_wpt.lane_id, s=0.0)
        #                 else:
    ...
    def run_step(self, debug=False):
        ...
        target_speed = self._overtake(ego_vehicle_loc, ego_vehicle_wp)
